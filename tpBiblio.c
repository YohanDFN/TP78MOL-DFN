...
{
int reponse,chx;
int pos; //utilisée dans la suppression d'un livre
T_Bibliotheque B; 
init( &B );
chargement(&B);
...
switch(chx)
	...
	case 3 :
        printf("Quel livre voulez vous rechercher (par Titre) : ");
        
        reponse=rechercherBibliotheque(&B);
        if (reponse==0) printf("Le livre n'existe pas");
      break;

  case 4 :
        reponse=rechercheAuteur(&B);
         if (reponse==0) printf("Le livre n'existe pas");
      break;
  ...
  case 5 :
  printf("Saisissez la position dans la bibilothèque du livre que vous voulez supprimer\n");
  scanf("%d",&pos);
  reponse=supprimerLivre(&B,pos);
  if(reponse==0) printf("Echec de la suppression !!!\n\n");
  else printf("Suppression réussie !!!\n\n");
